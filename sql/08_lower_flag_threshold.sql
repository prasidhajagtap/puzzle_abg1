-- ============================================================================
-- The Daily Nine — lower the cheat-flag line from 20 seconds to 10
--
-- WHY: the 20-second line was set as a guess at what a person could not do.
-- The live data says otherwise. Real players are solving in 13 to 18 seconds,
-- so the game's best players were being marked implausible for being good at
-- it, and the admin console's "needs review" list was filling up with them.
-- 10 seconds is the point where a run stops being fast and starts being hard
-- to believe.
--
-- WHAT CHANGES: exactly one number, and the comment above it.
--     was:  if p_solved and p_time_sec < 20 then
--     now:  if p_solved and p_time_sec < 10 then
--
-- WHAT DOES NOT CHANGE: nothing else at all. The scoring maths, the streak
-- rule, the clamps, the ON CONFLICT DO UPDATE that makes the last submitted
-- score the one that counts, SECURITY DEFINER and search_path are all
-- reproduced exactly as they stand today. This file was generated from
-- 01_migrate_submit_score_override.sql, which is the version that is applied,
-- so the only difference between them is the one number above.
--
-- NOT CHANGED HERE: submit_sprint_score also flags a run, at more than 20
-- puzzles cleared in ten minutes. That is a count of puzzles, not seconds,
-- and it is a different judgement. Left alone deliberately — say the word and
-- it can be revisited on its own evidence.
--
-- A flag has never blocked anything. It marks a row for a human to look at.
-- The grid is generated in the browser, so the server cannot replay the
-- puzzle and cannot prove a score either way. If prizes ever ride on these
-- boards, the real fix is generating the puzzle server-side and verifying the
-- answer; no threshold substitutes for that.
--
-- Safe to re-run. Rollback: 99_rollback_flag_threshold.sql
-- Verify: 09_verify_flag_threshold.sql
-- ============================================================================
