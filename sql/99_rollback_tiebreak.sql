-- ============================================================================
-- Rollback for 14_score_tiebreak.sql
--
-- STEP 1 restores submit_score exactly as 08 left it — the version that was
-- running before the tiebreak. It is byte-identical to that file. No score is
-- affected either way; time_ms was never an input to any of them.
--
-- STEP 2 drops the column, and is commented out on purpose. Restoring the
-- function is enough to stop new milliseconds being recorded; dropping the
-- column also throws away the ones already collected, and there is no reason
-- to do that unless the whole idea is being abandoned. A nullable column that
-- nothing reads costs nothing to leave in place.
-- ============================================================================
...
-- STEP 2 — destroys the collected milliseconds. Uncomment only if the tiebreak
-- is being abandoned entirely, not merely paused.
-- alter table public.scores drop column if exists time_ms;
